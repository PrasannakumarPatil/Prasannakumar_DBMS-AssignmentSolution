...
Select p.passenger_name, p.boarding_city, p.destination_city, p.bus_type, pr.price as Price  from passenger p inner join price pr on p.bus_type = pr.bus_type and p.distance = pr.distance group by p.passenger_name; 

-- 7) What is the passenger name and his/her ticket price who travelled in Sitting bus for a distance of 1000 KM s
Select pa.Passenger_name, pr.price from passenger pa inner join price pr on pa.bus_type = pr.bus_type and pa.distance = pr.distance where pa.bus_type = 'Sitting' and pa.distance = 1000;
-- No Passenger travelled in sitting bus for a distance of 1000kms

-- 8) What will be the Sitting and Sleeper bus charge for Pallavi to travel from Bangalore to Panaji?
Select pr.bus_type, pr.price from passenger pa, price pr where pa.passenger_name = 'Pallavi' and pa.distance = pr.distance;
